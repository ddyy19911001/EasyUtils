...
//                .addExceptionInterceptor(HttpInterceptor.ExceptionInterceptor)//请求链路异常拦截器
    ...
    public void sendMsgGet(String requestUrl,List<HeaderParam> params, JsonObject object, final Class x){
        if(object==null){
            object=new JsonObject();
        }
    ...
    public void sendMsgPost(String requestUrl,List<HeaderParam> params, JsonObject object, final Class x){
        if(object==null){
            object=new JsonObject();
        }
        HttpInfo requestInfo= getHttpInfoBuilder(requestUrl,object,params).build();
     ...
     * 异步上传单个文件：显示上传进度
     */
    public void doUploadSingleFile(String requestUrl, String fileName, File file, final Class x) {
        HttpInfo.Builder builder = HttpInfo.Builder();
        builder.setUrl(requestUrl)
     ...
     * 异步上传单个文件：显示上传进度
     */
    public void doUploadSingleFile(String requestUrl,List<HeaderParam> params, String fileName, File file, final Class x) {
        HttpInfo.Builder builder = HttpInfo.Builder();
        builder.setUrl(requestUrl)
                .addUploadFile(fileName, file, new ProgressCallback(){
                    @Override
     ...
     * 异步上传多个文件：显示上传进度
     */
    public void doUploadMuiltFiles(String requestUrl,FileListParams fileListParams, final Class x) {
     ...
     * 异步上传多个文件：显示上传进度
     */
    public void doUploadMuiltFiles(String requestUrl,List<HeaderParam> params,FileListParams fileListParams, final Class x) {
        HttpInfo.Builder builder = HttpInfo.Builder().setUrl(requestUrl);
        for(int i=0;i<fileListParams.getFiles().size();i++){
            builder.addUploadFile(fileListParams.getFileNames().get(i),fileListParams.getFiles().get(i).getAbsolutePath());
        }
